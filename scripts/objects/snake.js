...
MyGame.objects.Snake = function (
  ...
  snake.render = function () {
    ...
    const vectorX = snake.body[snake.body.length - 2].center.x - snake.body[snake.body.length - 1].center.x;
    const vectorY = snake.body[snake.body.length - 2].center.y - snake.body[snake.body.length - 1].center.y;

    let rot = ((Math.atan(vectorY / vectorX) - Math.PI / 2) + (Math.PI * 2)) % (Math.PI * 2);
    if(vectorX < 0) {
      rot += Math.PI
    }
