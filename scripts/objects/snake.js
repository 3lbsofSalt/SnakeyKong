...
MyGame.objects.Snake = function(spec) {
  ...
  for(let i = 0; i < spec.startingSegments; i++) {
    ...
    const lastLocation = (i == 0 ? {...snake.head.center} : {...snake.body[i - 1].center});
    ...
    snake.body.push(MyGame.objects.Body({
      size: { x: 50, y: 50 },       // Size in pixels
      center: { x: lastLocation.x - xDiff, y: lastLocation.y - yDiff },
      rotation: spec.rotation,
      moveRate: spec.moveRate,          // Pixels per second
      rotateRate: spec.rotateRate,    // Radians per second
      nextLocations: queue
  ...
  snake.moveForward = function(elapsedTime) {
    ...
    for(let i = 0; i < snake.body.length; i++) {
      ...
      snake.body[i].moveForward(elapsedTime, nextSegment, snake.segmentDistance);
    }
  }

  snake.ROTATION_TOL = Math.PI / 50;

  snake.setDirectionUp = function(elapsedTime) { snake.head.setDirectionUp(elapsedTime); }
  snake.setDirectionDown = function(elapsedTime) { snake.head.setDirectionDown(elapsedTime); }
  snake.setDirectionLeft = function(elapsedTime) { snake.head.setDirectionLeft(elapsedTime); }
  snake.setDirectionRight = function(elapsedTime) { snake.head.setDirectionRight(elapsedTime); }
  ...
  snake.updateRotation = function (elapsedTime) {
    ...
    } else { snake.head.rotation = snake.head.desiredRotation; }
  }
